	...
	if util.IsExist(localConfig) {
		...
		templateAction := action.Template{
			...
			Data: util.Data{
				...
				"ApiServerArgs":          templates.UpdateFeatureGatesConfiguration(ApiServerArgs, g.KubeConf, host.GetInternalAddress()),
				"EnableAudit":            g.KubeConf.Cluster.Kubernetes.EnableAudit(),
				"ControllerManagerArgs":  templates.UpdateFeatureGatesConfiguration(ControllerManagerArgs, g.KubeConf, host.GetInternalAddress()),
				"SchedulerArgs":          templates.UpdateFeatureGatesConfiguration(SchedulerArgs, g.KubeConf, host.GetInternalAddress()),
				"KubeletConfiguration":   templates.GetKubeletConfiguration(runtime, g.KubeConf, g.KubeConf.Cluster.Kubernetes.ContainerRuntimeEndpoint, g.WithSecurityEnhancement, host.GetInternalAddress()),
